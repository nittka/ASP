...
class ClassicalLiteralValidator extends AbstractDeclarativeValidator {
	...
	def private dispatch internalCheckHead(Disjunction j,Rule r){
		val errorLiterals= j.literal.filter[!validClassicalLiteral]
		errorLiterals.forEach[
			error("disjunction literal may only be a classical literal", j,ASPPackage$Literals::DISJUNCTION__LITERAL,j.literal.indexOf(it))
		]
	}
